...
class Kama_Make_Thumb {
	...
	protected function do_thumbnail(){
		...
		$img_str = $this->get_img_string();
		$size    = $img_str ? $this->_image_size_from_string( $img_str ) : false;
		...
		// Imagick
		if( 'imagick' === $use_lib ){
			$done = $this->make_thumbnail_Imagick( $img_str );
			$this->metadata['lib'] = 'Imagick';
		}
		// GD
		elseif( 'gd' === $use_lib ){
			$done = $this->make_thumbnail_GD( $img_str );
	...
	public function img(){
		...
		if( $this->width )  $attrs['width'] = 'width="'.  intval( $this->width )  .'"';
		if( $this->height ) $attrs['height'] = 'height="'. intval( $this->height ) .'"';

		$attrs['alt'] = 'alt="'. ($rg['alt'] ? esc_attr($rg['alt']) : '') .'"';

		if( $rg['title'] ) $attrs['title']  = 'title="'. esc_attr( $rg['title'] ) .'"';
		if( $rg['class'] ) $attrs['class']  = 'class="'. preg_replace('/[^A-Za-z0-9 _-]/', '', $rg['class'] ) .'"';
		if( $rg['style'] ) $attrs['style']  = 'style="'. strip_tags($rg['style']) .'"';
		if( $rg['attr'] )  $attrs['custom'] = $rg['attr'];

		$attrs['lazy'] = 'loading="lazy"';

		$out = sprintf( '<img src="%s" %s>', esc_url( $src ), implode(' ', $attrs) );
